...
const AddToCart = ({ gameId, gameName, gameBg, type = "" }) => {
  ...
  return (
    <button
      disabled={isAdded}
      onClick={handleAddToCart}
      className={`absolute bottom-0 left-[50%] transform-gpu translate-x-[-50%] z-50 hidden group-hover:block mb-2 cursor-pointer px-2 py-1 text-xs rounded ${isAdded ? "cursor-not-allowed opacity-70 bg-gray-400" : "cursor-pointer bg-[rgba(51,134,243,0.5)] hover:bg-[rgba(51,134,243,0.97)]"}`}
    >
      {btnText}
    </button>
  );
