...
const SearchSection = () => {
  ...
  const debouncedSearchTerm = useDebounce(searhTerm, 500);
  ...
  return (
    <div className="max-w-[1200px] m-auto flex items-center justify-between p-4 gap-6 ">
      <div
        className="flex items-center gap-2 bg-gray-900 rounded-4xl p-2 px-4 relative w-full max-w-[400px]"
        ref={searchRef}
      >
          ...
          >
            {searchResults.map((item) => (
              <Link
                ...
                key={item.id}
                to={`./game/${item.id}`}
                className="p-2 px-4 hover:bg-zinc-800 rounded text-gray-400 hover:text-gray-50"
              >
                {item.name}
              </Link>
            ))}
