...
import { useRemoveProductFromCartMutation } from "../features/cart/cartApi";

const CartItem = ({ game }) => {
  const [removeProductFromCart, isLoading, isSuccess] =
  ...
  const handleRemoveFromCart = async () => {
    ...
    await removeProductFromCart(game.game_id);
  };

  return (
    <div className="p-4 flex items-center gap-6 hover:bg-gray-700 rounded transition hover:scale-101 group">
      <div
        className="h-20 aspect-square rounded overflow-hidden bg-cover bg-center"
        style={{ backgroundImage: `url('${game.game_bg}')` }}
      >
      ...
      <div className="opacity-50 group-hover:opacity-100 flex flex-col gap-1 sm:gap-2 w-full">
        <div className="font-bold sm:text-xl">
          {game.game_name.length > 12
            ? game.game_name.slice(0, 11)
            : game.game_name}
        </div>
        <div className="flex flex-col gap-2 md:flex md:flex-row justify-between">
          ...
          <div className="flex gap-4">
            <Link
              ...
              className="cursor-pointer py-1 px-4 bg-blue-700 rounded w-fit"
            >
              Visit
            </Link>
            <button
              ...
              className="cursor-pointer py-1 px-4 bg-red-700 rounded w-fit"
            >
              {removeBtnText}
            </button>
          </div>
