...
const GameCard = ({ game }) => {
  ...
  const getPlatformIcon = (platformName) => {
    ...
    if (name.includes('ios') || name.includes('android') || name.includes('mobile')) return '📱';
    return '🎮';
  };
  ...
  useEffect(() => {
    let interval;
    if (isHovered && game.short_screenshots && game.short_screenshots.length > 1) {
      interval = setInterval(() => {
        setCurrentScreenshot((prev) =>
          (prev + 1) % game.short_screenshots.length
        );
      }, 1000);
    } else {
  ...
  return (
    <div
      className="relative group rounded-lg overflow-hidden bg-[#1a1a1a] shadow-lg transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl hover:shadow-purple-500/20 cursor-pointer max-w-[250px]"
    ...
    >
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        <img
          ...
          style={{
            transition: isHovered ? 'transform 0.5s ease, opacity 0.3s ease' : 'transform 0.5s ease'
          }}
        />
