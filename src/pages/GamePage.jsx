...
const GamePage = () => {
  ...
  return (
    <div className="max-w-[1200px] m-auto p-4 flex flex-col">
      <ScrollToTop />
      <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-white my-4">
        {game.name}
      </h1>
      <div className="grid lg:grid-cols-[5fr_2fr] gap-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="flex flex-col gap-4">
            <button className="py-2 max-w-full bg-white text-black rounded">
              Purchase
            </button>
            <AddToCart
              gameId={game.id}
      ...
      <button
        onClick={handleDes}
        className={`text-blue-700 text-sm ml-auto cursor-pointer ${game.description.length < 250 ? "hidden" : ""}`}
      >
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {game.genres.map((item) => {
            return (
              <div
                key={item.id}
                className="relative flex-1 min-w-[100px] max-w-[250px] basis-[200px]
                  ...
                  <div className="flex flex-1 min-w-[70px] max-w-[150px] basis-[100px] rounded-full overflow-hidden aspect-square relative">
                    ...
                    <div
                      className="absolute inset-0 
                                        ...
                                        text-xl font-bold"
                    >
                      {item.store.name.length > 12
                        ? item.store.name.slice(0, 12)
                        : item.store.name}
                    </div>
                  </div>
                </a>
